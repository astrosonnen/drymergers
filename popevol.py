...
class population:
    ...
    def __init__(
                self, z_0=2., nobj=100, mstar_chab_0=None, mhalo_0=None, veldisp_0=None, gammap_0=None):
    ...
    def evolve(self, ximin=0.03, dz=0.01, z_low=0., imf_recipe='mstar-vdisp', imf_coeff=(0.2, 0.2, 0.5), \
               vdisp_coeff=(2.40, 0.18), do_gammap=False, v200sigma_rat2 = 0.65, vorbv200_rat = 1.1):
            ...
            for j in range(self.nobj):
                ...
                def integrand(xi):
                    ...
                    thing = recipes.satellite_imf(lmstar_here, z=self.z[i], recipe=imf_recipe, coeff=imf_coeff, \
                            ...
                            rfunc(xi*self.mhalo[j, i])*xi**(beta+1.)*np.exp((xi/xitilde)**gamma)
                    return thing
                ...
                if do_gammap:
                    igz = quad(
                    lambda xi: fgfunc(xi)*rfunc(xi*self.mhalo[j, i])*xi**(beta+1.)*np.exp((xi/xitilde)**gamma), ximin, 1.)[0]
